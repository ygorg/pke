...
class YAKE2(LoadFile):
    ...
    def candidate_weighting(self, window=2, stoplist=None, use_stems=False, ecir_weight=False, pool_best=False):
        ...
        if not use_stems:
            # If using the lowercase form as the lexical form
            # Add a `lower` attribute in the sentences for easy access
            for s in self.sentences:
                setattr(s, 'lower', [w.lower() for w in s.words])

            # Create alternate `self.candidates` where keys are lowercase forms
            # instead of the stemmed ones.
            self.stemmed_candidates = self.candidates
            self.candidates = defaultdict(Candidate)
            for stemmed_form, c in self.stemmed_candidates.items():
                cand = zip(c.surface_forms, c.pos_patterns,
                           c.offsets, c.sentence_ids)
                for surface_form, pos_pattern, offset, sentence_id in cand:
                    lowercased_tokens = [w.lower() for w in surface_form]
                    self.add_candidate(surface_form, lowercased_tokens,
                                       pos_pattern, offset, sentence_id)
                    # Also adding a `stemmed` attribute for the lowercase
                    #  candidates to ease the conversion to stemmed candidates
                    setattr(self.candidates[' '.join(lowercased_tokens)],
                            'stemmed', stemmed_form)
            ...
            # use words
                ...
                for j, token in enumerate(tokens):
                    if self.features[token]['isstop']:
                        ...
                        if j - 1 >= 0:
                            term_l = tokens[j - 1]
                            prob_t1 = self.contexts[term_l][1].count(term_stop)
                            prob_t1 /= self.features[term_l]['TF']
                        if j + 1 < len(tokens):
                            term_r = tokens[j + 1]
                            prob_t2 = self.contexts[term_stop][0].count(term_r)
                            prob_t2 /= self.features[term_r]['TF']
    ...
    def get_n_best(self, n=10, redundancy_removal=True,
                   stemming=False, threshold=None):
        # TODO: Very hacky, what could we do to better use inheritance
        #       in this case ?
        # The paper describes the use of a threshold to compute the redundancy
        if threshold:
            self._threshold = threshold
        res = super().get_n_best(n, redundancy_removal, stemming)
        if threshold:
            del self._threshold
        return res

...
@Model.register("seq2seq_copy")
class CopyNetSeq2SeqKE(Model):
    """
    ...
    def __init__(self,
                 ...
                 text_based_metrics: List[Metric] = None,
                 filter_output: bool = False) -> None:
        ...
        self._text_based_metrics = text_based_metrics
        ...
        self._decoder_type = decoder_cell.lower()
        if decoder_cell == 'gru':
            cell = GRUCell
        elif decoder_cell == 'lstm':
            cell = LSTMCell
        else:
            raise ConfigurationError('decoder_cell must be GRU or LSTM')
        self._decoder_cell = cell(self.decoder_input_dim, self.decoder_output_dim)
    ...
    def forward(self,  # type: ignore
        ...
        output_dict['id'] = [x['id'] for x in metadata]
        ...
        if not self.training:
            ...
            if target_tokens:
                ...
                if self._text_based_metrics is not None:
                    predicted_tokens = self._get_predicted_tokens(output_dict["predictions"],
                                                                  metadata)
                    for metric in self._text_based_metrics:
                        metric([[' '.join(tokens) for tokens in kp] for kp in predicted_tokens],  # type: ignore
                               [x['ref'] for x in metadata])
    ...
    def _decoder_step(self,
        ...
        if self._decoder_type == 'lstm':
            decoder_hidden, decoder_context = decoder_hidden
        ...
        for top_k_predictions, metadata in zip(predicted_indices, batch_metadata):
            ...
            for indices in top_k_predictions[:n_best]:
                ...
                for index in indices:
                    ...
                    tokens.append(token)
                if self._filter_output and\
                        not {self._start_index, self._end_index,
                             self._oov_index, self._pad_index,
                             self._copy_index}.intersection(set(tokens)):
